...
if (existe == false) {
    ...
    public static void Eliminar_estudiante() {//Elimina los datos del estudiante por medio de la cedula
        Scanner reader = new Scanner(System.in);
        System.out.println("Por favor, ingrese la cedula del estudiante que requiere eliminar: ");
        int ID = reader.nextInt();
        for (int i = 0; i < cedula.length; i++) {
            if (ID == cedula[i]) {
                cedula[i] = 0;
                ...
                promedio[i] = 0;
                condición[i] = "";
                System.out.println("Los datos del estudiante fueron eliminados");
